...
public class Wandering extends BaseComponent {
    ...
    public Wandering(BaseComponent component) {
        ...
        this.requestPredicate = component.requestPredicate;
    ...
    public void start() {
        ...
        while (this.isRun) {
            ...
            if (Objects.nonNull(request)) {
                consumer(request);
                EXECUTOR_SERVICE.submit(() -> handler(request));
                waitTime = 0;
            }
    ...
    private synchronized void initComponent() {
        ...
        if (Objects.isNull(EXECUTOR_SERVICE)) {
            EXECUTOR_SERVICE = GuavaThreadPoolUtils.getGuavaExecutor(threadCount);
        }
    ...
    @Override
    public Wandering addEventRequest(Predicate<Request> predicate, Consumer<Request> consumer) {
        super.addEventRequest(predicate, consumer);
        return this;
    }

    @Override
    public Wandering addEventResponse(Predicate<Response> predicate, Consumer<Response> consumer) {
        super.addEventResponse(predicate, consumer);
        return this;
    }
